...
class SeasonIndex extends Component
{
    ...
    {
        if(Season::where('season_number', $this->seasonNumber)->exists()){
            $this->dispatchBrowserEvent('banner-message', ['style' => 'danger', 'message' => 'Season exists']);
            return;
        }
        $apiSeason = Http::get('http://api.themoviedb.org/3/tv/' . $this->serie->tmdb_id . '/season/' . $this->seasonNumber . '?api_key=43d395118985f519a8036c48a693099c&language=en-US
                    ');
        if ($apiSeason->ok()) {
